...
def update_position(envMode = False):
  global agent_x
  global agent_y
  global velocity
  global angular_velocity
  global direction
  candidate_x = agent_x + math.cos(direction)*velocity;
  candidate_y = agent_y + math.sin(direction)*velocity; # never more than 1 pixel away. 
...
def accelerate(): # Max at 1 makes learning easier, and makes update steps less involved. 
  global velocity
  velocity = min(velocity + 0.04, 1);
...
def swivel_clock():
  global angular_velocity
  angular_velocity = min(angular_velocity + 0.01, 0.01);


def swivel_anticlock():
  global angular_velocity

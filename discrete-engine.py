...
def action_left(lim=10, envMode = False):
  global agent_x
  agent_x -= biggest_step(lim, lambda step : (agent_x - step, agent_y))
  universal_update(envMode)

def action_right(lim=10, envMode = False):
  global agent_x
  agent_x += biggest_step(lim, lambda step : (agent_x + step, agent_y))
  universal_update(envMode)

def action_down(lim=10, envMode = False):
  global agent_y
  agent_y += biggest_step(lim, lambda step : (agent_x, agent_y + step))
  universal_update(envMode)

def action_up(lim=10, envMode = False):
  global agent_y
  agent_y -= biggest_step(lim, lambda step : (agent_x, agent_y - step))
  universal_update(envMode)
